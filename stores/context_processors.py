    ...
    if numbers:
        ...
        if room is not None:
            if request.path == f"/store/{numbers[0]}/services/":
                ...
                cart_count = Cart.objects.filter(room=room).count()
        else:
            cart_count = 0
...
def notification_count(request):
    if request.user.is_authenticated:
        try:
            notification = Notification.objects.filter(room__user=request.user, is_readed=False)[:5]
            notification_count = notification.count()
            notification_data = NotificationSerializer(notification, many=True).data
        except Notification.DoesNotExist:
            notification_count = 0
        
        return {'notification_count': notification_count, 'notifications': notification_data}
    else:
